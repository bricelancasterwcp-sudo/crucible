...
| **Δ_min (endpoint threshold, §6 — derived at lock)** | **0.0884 (≈ 8.8 pp)** = 2·√(2·p0(1−p0)/C) with p0 = 8/30, C = 200; sanity band [5 pp, 12 pp] ✓ |
| Adapter accept gate (§5; distinct from Δ_min) | `ACCEPT_MAX_DROP = 1` on a `min(12, |solved|)` holdout slice [amendment A4] |
...
| Gating task set | `--tasks all` — the full 450 (endpoints E1/E5 need phase-2 and novel exposures) |
