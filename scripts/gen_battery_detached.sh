...
# amendment, controller ruling round 2; PARALLELIZED round 3). Mirrors
# gen_minority_detached.sh's detach/pid/log/DONE mechanics and refusal shape
# (OUT must ALREADY exist and already contain functions.jsonl; refuses if
# OUT/battery_stats.json already exists -- one pass only), but launches NO
# proposer/server at all: this pass is a fixed, no-model enumeration over
# each function's own arity (crucible.latent.gen_battery.
# generate_battery_inputs), replacing the LLM-proposed minority pass after
# its live-fire failure (93% parse_fail, 0 accepted_minority -- see
# runs/blite-corpus/minority_stats.llm-attempt.json and
# crucible/latent/gen_battery.py's module docstring). Round 3 (sizing:
# ~60k harvests sequential ~= 11h, untenable) parallelized the harvest step
# with a ThreadPoolExecutor(jobs) internally -- JOBS below, default 8, same
# knob/default as reharvest_detached.sh's own $2. The `timeout 7200` is this
# pass's own budgeted window (2h -- generate_battery_inputs trusts the
# caller's bound, same as its siblings).
#
# Usage: gen_battery_detached.sh <out-dir> [jobs]
set -u
...
stats = generate_battery_inputs(Path(\"'"$OUT"'\"), seed=0, jobs='"$JOBS"')
...
echo "launched battery-pass out=$OUT jobs=$JOBS pid=$(cat "$OUT".battery.pid)"
